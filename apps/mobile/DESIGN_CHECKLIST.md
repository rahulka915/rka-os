...
- [x] `AreaDetailScreen.tsx` — redesigned 2026-08-06 as a chapter-style Domain detail: full-bleed River Stone header reusing the existing Journey landscape art (tinted, no new asset), score + a maintenance-vs-current directional cue (no fabricated time-trend — none is stored), Missions/Pillars/Achievements sections restyled to brass/vermilion with accessibility labels added throughout, Pillar rows now surface contributing habits inline. Refined 2026-08-10 with an explicit 220pt artwork boundary (preventing the portrait source from expanding), custom Domain identity and title-aware Skill marks.
- [ ] `AreasScreen.tsx` — list of life areas
- [x] `CalendarScreen.tsx` — refined 2026-08-11: the Month view is compact, retains selection in-place, and reveals a selected-day mini agenda rather than forcing the Timeline. Timeline is deliberately one selected day (header arrows step by day) and uses a clean single-column grid: the old between-day wave, `Block now` header and category-lane icon strip are gone. The header is a compact date control with an iOS-style segmented Calendar/Timeline switch. `00:00` is fully visible and `23:59` closes the day; events are readable compact blocks, not floating pins. It uses an iOS-style River Stone planning drawer (collapsed above the global dock, expanded as a lower-half sheet) for Unscheduled + Anytime Today. Its items are calm queue rows with type/status labels and a visible drag grip, rather than repeated heavy cards. The existing long-press drag-to-time, 15-minute snapping and visible blue Now line are preserved; brass leads selection and vermilion is reserved for the add action.
...
- [x] `PotentialScreen.tsx` — redesigned 2026-08-06: River Stone hero card wrapping the new `HaradaWheel` (see below), restyled Current Focus/Domains/Achievements rows to brass/vermilion, Domains list kept as the accessible flat fallback below the wheel. Same `computeDomainScore`/`computeOverallPotential`/`getFocus` calls as before — no scoring logic touched.
- [x] `FocusScreen.tsx` — token-level restyle 2026-08-06 (brass/vermilion, filled River Stone-toned rows, 44pt weight-input target) — no functional changes.
...
- [ ] `ProfileScreen.tsx` — user profile + dev bench. Partial 2026-08-06: only the Potential card's accent tokens restyled (vermilion/brass) as part of the Journey/Domains redesign — the header and the rest of the screen untouched, still pending its own full pass.
...
- [x] `JourneySummaryStrip.tsx` (new, 2026-08-06) — compact Overall Potential + Current Focus readout rendered directly below the hero card; the hero itself stays about today's task completion, unchanged by this addition.
