...
**Progression visual identity:** The eight canonical Domains use the custom transparent PNG family in `src/components/icons/DomainIcons.tsx` through `src/utils/domainIcons.ts` on onboarding, Domains, Domain Detail and Harada/Potential surfaces. Runtime art lives in `assets/icons/domains/`; Discipline uses the wrapped mala replacement, and Overall Potential alone keeps the bonsai/custom fallback. All Missions use the same generated mountain destination identity through `ProjectPortfolioIcon`/`ProjectPlaceholderIcon` because they are user-created projects at scale; per-Mission artwork/emoji is intentionally not shown in the Mission list. Skills use `SkillIdentityIcon.tsx` for title-aware identity marks and show the five human-readable proficiency stages rather than raw percentages in the Skill grid.
...
**Pillars (product term for `potential-stat`):** User-facing copy everywhere says **Pillar/Pillars**; the internal item `type` is still `'potential-stat'` and every code symbol (`getPotentialStats`, `metadata.potentialStat`, `PotentialStatResult`, relation `'potentialStatArea'`, `createPotentialStat`, `migratePotentialStats`, …) is unchanged — do **not** rename them, there is no migration. Pillars are **optional, first-class maintenance/capacity areas, mostly Health & Fitness** — NOT a required child of every Domain. A Domain with no linked Pillars is not "failing": `domainMaintenance(percents)` in `src/utils/domainScoring.ts` returns `NO_PILLAR_MAINTENANCE_BASELINE` (10) as a neutral floor when there are no Pillars, so Mission/Achievement/Skill lift floats above it and it doesn't drag Overall Potential to 0. Domains *with* Pillars keep the original average-of-habit-streaks maintenance. Native `computeDomainMaintenance` (`database.ts`) and web `computeDomainScoreApprox` (`PotentialOverview.web.tsx`) both route through the shared helper. `SUGGESTED_PILLARS` (`src/utils/potential.ts`, grouped Health & Wellbeing / Fitness & Performance) is offered as optional quick-adds in `AreaDetailScreen.tsx`'s "Add Pillar" sheet — nothing is auto-created (the legacy 4-stat `migratePotentialStats` seed is left untouched on purpose). Finance/Relationships/Discipline/Growth/Creativity/Career are expected to have no Pillars. **Pillars now also has its own standalone page** (`PillarsScreen.tsx`/`PillarsScreen.web.tsx`, see below) — reachable via the native "More" grid / web sidebar, not just by drilling into a Domain.

**Actions model (shipped, non-scoring):** An Action is a lightweight `activityLogs` row (`actionType: 'action'`), not a new item type — `logAction`/`getActions`/`updateAction`/`deleteAction` in both `database.ts` and `database.web.ts`, typed via `src/utils/actions.ts` (`ActionDetails`: title/kind `'practice'|'general'`/durationMinutes/intensity/why/optional Domain-Pillar-Skill-Mission links). This is the first place skill practice with duration/intensity/"why" is recordable — `computeSkillPracticeSummary` still only *derives* activity read-only from linked habit/routine completions, unrelated to this. `getActionFeed` (both DB files, built on the pure `buildActionFeed` in `utils/actions.ts`) is a unified read-only feed merging logged Actions with habit check-ins/task completions/medication doses/routine steps, newest-first. **Scoring-safe by design:** logged Actions never write `domainContributions`, touch skill proficiency, or affect any existing calc — recorded + displayed only, feeding proficiency/score is explicitly deferred. UI: `ActionsScreen.tsx`/`ActionsScreen.web.tsx` (log control + feed), standalone destination on both targets (native "More" grid + `MenuStack.tsx`; web sidebar `PROGRESSION_ITEMS` + `AppShell.web.tsx`). See `docs/superpowers/specs/2026-08-13-pillars-and-actions-design.md` for the full spec.
