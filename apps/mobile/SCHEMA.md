...
| `weight` | TEXT | `'minor'` \| `'moderate'` \| `'major'` — the only user-facing contribution strength for v1; hidden numerical magnitudes live in the per-Attribute `AttributeScoringConfig.weightMagnitude` (`utils/attributeScoring.ts`), tunable independently of this table |
| `fraction` | REAL? | proportional credit, 0..1 — NULL means full credit (1). Set only for measurable (count/duration) Habits (2026-08-15), reflecting actual/target progress at the time it was last recomputed for the current period. Applied once at the raw-unit level (`fraction × weightMagnitude`), never through the weekly-credit curve a second time. |
| `occurredAt` | INTEGER | epoch ms the evidence happened at |
| `excludedAt` | INTEGER? | soft-disable (NULL = active) — used when an Action is edited (old tags excluded, current ones re-inserted), or when a measurable Habit's current-period evidence is replaced (see below) |
| `createdAt` | INTEGER | epoch ms |

Evidence-generation call sites: for binary Habits, `updateItemStatus`'s repeating-completion branch and `toggleHabitOccurrence`'s add branch (both via `recordHabitCompletionEvidence`, one-shot, `fraction` unset). For measurable (count/duration) Habits (2026-08-15), `logHabitSample`/`undoLastHabitSample` both call `recordHabitProgressEvidence`, which recomputes the *current period's* completion fraction from scratch every time (via `utils/habitMeta.ts`'s `computeHabitPeriodProgress`/`periodWindow` — the same period math the progress UI already uses) and **replaces** that period's evidence rather than accumulating it: it soft-excludes whatever this Habit already recorded within the current period window, then inserts one fresh row per configured Attribute at the freshly-computed fraction — so repeated same-day progress updates (3k → 6k → 9k → 12k steps) never stack duplicate evidence, and a downward change (undo) is reflected correctly. Earlier, already-finished periods are untouched. For Actions, `logAction`/`updateAction`/`deleteAction` (reading `ActionDetails.attributeContributions`, `fraction` unset — Actions have no measurable-progress notion). A Habit's Attribute config is independent of and unrelated to its legacy single-target `metadata.potentialStat`. Web mirror: both tables are localStorage-backed (`rka-os:attributeDomains`/`rka-os:attributeContributions`), same "no firestoreWebStore mirror yet" caveat as `dailyCheckIns` — not cross-device synced yet. Web-specific note: since Firestore writes are async with no local optimistic update, `logHabitSample`/`undoLastHabitSample` construct the up-to-date sample list by hand rather than re-querying immediately after a write (which would read stale data) — see the inline comments at those functions in `database.web.ts`.
