...
**Agentic AI assistant — web-ahead divergence (2026-08-15):** The AI assistant (`AssistantOverlay`, opened via the floating Sparkles button in `AppShell.web.tsx`) is **agentic on web only**. On web (`services/ai/assistant.web.ts` + `assistantTools.ts`/`assistantToolExecutor.ts`) it can create/update/complete/delete items, log habit samples/medication doses/actions, and — as of this pass — create Missions/Skills/measurable Habits, link items, set Focus, and run a conversational "Set up my system" onboarding, all via Gemini (Vertex AI backend, `gemini-2.5-flash`) function calling with per-item confirm-before-write. **Native's `services/ai/assistant.ts` stays read-only** (answers questions, no tool calls) — a deliberate, tracked divergence, not an accidental gap. Closing it later means giving native an agentic loop + a web-independent SQLite context builder. See `docs/superpowers/specs/2026-08-15-agentic-web-assistant-design.md` and `2026-08-15-conversational-onboarding-design.md`.
...
| To Get | ✅ | ✅ | ✅ | Close. |
| Medications & Supplements | ✅ | ✅ | 🟡 | Web missing focus timeline, too-soon override, eligibility/streak, reminders (medications only). **Supplements** (2026-08-14, new): both targets merge a "Supplements" section into this screen — name/dose, optional electrolyte nutrient profile (sodium/potassium/magnesium/calcium/chloride), tap-to-log, and a daily nutrient-totals summary strip. Deliberately at parity from launch — no stock/timer/Live-Activity machinery on either target, by design (see `SCHEMA.md`'s "Supplement nutrient tracking"). |
