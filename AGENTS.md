...
**Current desktop web app:** `apps/mobile/src/webApp/` + `App.web.tsx`, run via `npm run web`. Screen parity now includes Home, Inbox, Tasks, Areas/Missions, Calendar, Upcoming, Archive, Objects (To Get), Medications, Workouts (+ Exercise Library, Workout Template detail, Trends), Habits, Settings, Potential/Profile ("Me"), Achievements, Focus, Skills, Routines, Plan Backwards, and Daily Check-In/Daily Log. Remaining notable gaps are tracked in `apps/mobile/WEB_PARITY.md` (especially Workouts live session logging, Routine session play, Daily Check-In cross-device sync, medication focus/reminder extras, and web-only scoring simplifications).
...
**Progression icon system (shipped):** `apps/mobile/src/components/icons/DomainIcons.tsx` wraps the custom transparent PNG artwork under `apps/mobile/assets/icons/domains/` for the fixed eight Domains, resolved app-wide by `src/utils/domainIcons.ts`; the bonsai is reserved for Overall Potential/custom fallback, and Discipline uses the wrapped mala replacement rather than the earlier target draft. Missions currently resolve through the same domain-icon wrapper to `collection-missions-mountain.png` via `ProjectPortfolioIcon`/`ProjectPlaceholderIcon`. Skills remain identity-led through `SkillIdentityIcon.tsx`, which selects a recognisable capability mark from the Skill title.

**Current collection artwork:** `apps/mobile/src/components/icons/CollectionIcons.tsx` wraps the transparent PNG destination artwork for app destinations. The latest generated app-area PNGs live under `apps/mobile/assets/icons/domains/collection-*.png` for Habits, Routines, Skills, To Get and Workouts; Archive, Potential and Achievements still use the approved files under `apps/mobile/assets/icons/collections/`. Reuse these artwork components instead of generic entity glyphs; keep system icons for universal actions.
...
**Muscle group icon staging:** custom muscle-group artwork belongs under `apps/mobile/assets/icons/muscle-groups/` using transparent PNGs named for the eight `MUSCLE_GROUPS` keys (`chest`, `back`, `shoulders`, `arms`, `legs`, `core`, `full-body`, `cardio`). Keep these separate from `assets/exercises/`, which is the generated starter-exercise thumbnail library.

**Current Home journey prototype:** `apps/mobile/src/components/home/RoninJourneyPrototype.tsx` renders the Ronin through fixed-canvas PNG frames over the parallax sky. The runtime idle library is `apps/mobile/assets/ronin/idle-v2/`: calm breathing plus weighted look-around, blink, yawn, wrap-adjustment and shoulder-roll clips after random 8–18 second calm intervals. Walking and explicit jump/bow actions interrupt personality idles; each special clip returns to calm before another interval. Reduce Motion retains the complete idle library while suppressing the extra outer rotation and walk bob. `roninSpriteRegistry.ts` is the static asset registry and `roninIdleScheduler.ts` owns selection/delay rules.

**Current Calendar/Timeline direction:** `apps/mobile/src/screens/CalendarScreen.tsx` keeps the existing long-press, drag-to-time, 15-minute snap workflow, but surfaces unscheduled work through a collapsed/expanded bottom planning drawer rather than a full-height tray. The Timeline deliberately represents one selected day as one clean, single-column grid; header arrows move it day by day, all hour boundaries are visible from `00:00` to `23:59`, and scheduled work uses compact labelled blocks. The Calendar month is intentionally compact and retains the selected day's mini agenda; selecting a date does not force a mode switch.

**Current Domains progress treatment:** `apps/mobile/src/screens/AreasScreen.tsx` uses quiet empty progress tracks for 0% Domains (`RiverStoneProgress showZeroFill={false}`) so empty Domains do not show vermilion warning nubs. Actual Domain progress uses restrained brass; vermilion is reserved for Focus/active emphasis.

**Current material-language baseline:** production native surfaces use the approved warm-depth River Stone pass in `apps/mobile/src/components/riverstone/riverStoneTokens.ts` + `apps/mobile/src/components/riverstone/materials.ts`, with synced dock/header constants in `apps/mobile/src/theme/riverStone.ts`: cool sumi background, graphite River Stone cards/rows, blackened-iron-leaning chrome, sparse brass/vermilion accents and semantic-colour icons. `apps/mobile/src/components/dev/MaterialSheetWorkbench.tsx` remains available only through Settings → DEV TOOLS in `__DEV__` for tuning Washi/Sumi, River Stone, Blackened Iron, Urushi Lacquer and Gold/Brass roles before further production changes.

**Retained Rive journey runtime:** `@rive-app/react-native`, `react-native-nitro-modules`, `apps/mobile/assets/rka_journey_rig.riv`, and `RoninJourneyRiveWalker.tsx` remain available for the separate rig experiment, but Home currently mounts the PNG-frame renderer described above. Rive contains native code, so test that retained renderer in a freshly regenerated development build rather than Expo Go.
...
**Pillars (`potential-stat`):** User-facing term for `potential-stat` items is **Pillar/Pillars** — the internal `type` and all code symbols stay `potential-stat`/`potentialStat` (no migration; don't rename them). Pillars are **optional, first-class maintenance areas, mostly Health & Fitness** — not a required child of every Domain. A Pillar-less Domain isn't "failing": `domainMaintenance()` in `apps/mobile/src/utils/domainScoring.ts` gives it a neutral `NO_PILLAR_MAINTENANCE_BASELINE` (10%) floor instead of 0%. A future **Actions** model (Actions umbrella → Tasks/habit check-ins/skill-practice logs, each optionally answering "why?" and linking to Domain/Pillar/Skill/Mission) is documented product direction only, not shipped — see `apps/mobile/CLAUDE.md`.

**Journey/Domains/Missions/Potential scoring:** Domain scores = live maintenance baseline (from linked `potential-stat`/Pillar items, or the 10% floor when none are linked) + a capped, decaying "achievement lift" from `domainContributions` rows; Overall Potential is a weighted average of Domain scores, weighted by the active `focus` item. Manually-added Achievements must call `setAchievementContributesToScore` to actually create their scoring row — `createAchievement` alone only stores the flag on the item, it does not touch `domainContributions` (fixed 2026-08-05 for the Achievements screen's retrospective add flow, which previously silently no-op'd). See `apps/mobile/SCHEMA.md` (canonical schema/formula reference) and `apps/mobile/src/utils/domainScoring.ts` (pure scoring math) before touching any of this.
